...
CONFIGS = {
 'full':        {'chm':1,'dtm':1,'rgb':1,'ir':1,'sen':1,'aux':1,'chmstats':1},
 'chm_only':    {'chm':1,'dtm':0,'rgb':0,'ir':0,'sen':0,'aux':1,'chmstats':1},
 'no_dtm':      {'chm':1,'dtm':0,'rgb':1,'ir':1,'sen':1,'aux':1,'chmstats':1},
 'no_ortho':    {'chm':1,'dtm':1,'rgb':0,'ir':0,'sen':1,'aux':1,'chmstats':1},
 'no_s2':       {'chm':1,'dtm':1,'rgb':1,'ir':1,'sen':0,'aux':1,'chmstats':1},
 'no_chmstats': {'chm':1,'dtm':1,'rgb':1,'ir':1,'sen':1,'aux':1,'chmstats':0},
 'no_aux':      {'chm':1,'dtm':1,'rgb':1,'ir':1,'sen':1,'aux':0,'chmstats':1},
}
CFG = None   # set in main()
GN_GROUPS=8
BATCH=32; LR=1e-4; EPOCHS=80; DROPOUT=0.1   # V2 best epoch was 54; restart at 60 was destructive
...
class DS(Dataset):
    ...
    def __getitem__(self,i):
        ...
        # CHM always read (needed for chm_stats even if stream is off)
        chm=load_band(f"{DATA}/CHM/{npp}.tif",BAND_MAX['CHM'],PATCH['CHM'])
        if CFG['dtm']: dtm=load_band(f"{DATA}/DTM/{npp}.tif",BAND_MAX['DTM'],PATCH['DTM'])
        else:          dtm=np.zeros((1,PATCH['DTM'],PATCH['DTM']),dtype=np.float32)
        if CFG['rgb']: rgb=load_band(f"{DATA}/ORTHO/{npp}.tif",BAND_MAX['ORTHO'],PATCH['RGB'],bands=[1,2,3])
        else:          rgb=np.zeros((3,PATCH['RGB'],PATCH['RGB']),dtype=np.float32)
        if CFG['ir']:  ir =load_band(f"{DATA}/ORTHO/{npp}.tif",BAND_MAX['ORTHO'],PATCH['IR'],bands=[4,3,2])
        else:          ir =np.zeros((3,PATCH['IR'],PATCH['IR']),dtype=np.float32)
        if CFG['sen']: sen=load_band(f"{DATA}/S2/{npp}.tif",BAND_MAX['SEN'],PATCH['SEN'])
        else:          sen=np.zeros((12,PATCH['SEN'],PATCH['SEN']),dtype=np.float32)
        aux = chm_stats(chm) if CFG['chmstats'] else []
...
class Model(nn.Module):
    def __init__(self,nt,na,cfg=None):
        super().__init__()
        c = cfg if cfg is not None else CONFIGS['full']
        self.cfg=c; fin=0
        self.chm=stream(1) if c['chm'] else None; fin+=512 if c['chm'] else 0
        self.dtm=stream(1) if c['dtm'] else None; fin+=512 if c['dtm'] else 0
        self.ir =stream(3) if c['ir']  else None; fin+=512 if c['ir']  else 0
        self.rgb=stream(3) if c['rgb'] else None; fin+=512 if c['rgb'] else 0
        self.sen=sen_stream() if c['sen'] else None; fin+=256 if c['sen'] else 0
        self.aux=None
        if c['aux']:
            ...
            fin+=128
        assert fin>0, "no active streams" 
    ...
    def forward(self,chm,dtm,ir,rgb,sen,aux):
        f=[]
        if self.chm is not None: f.append(self.chm(chm))
        if self.dtm is not None: f.append(self.dtm(dtm))
        if self.ir  is not None: f.append(self.ir(ir))
        if self.rgb is not None: f.append(self.rgb(rgb))
        if self.sen is not None: f.append(self.sen(sen))
        if self.aux is not None: f.append(self.aux(aux))
        return self.fus(torch.cat(f,1))
...
def main():
    global CFG
    import argparse
    ap=argparse.ArgumentParser(); ap.add_argument('--config',required=True,choices=list(CONFIGS.keys()))
    a=ap.parse_args(); CFG=CONFIGS[a.config]; TAG=a.config
    os.makedirs(EXPORT,exist_ok=True)
    print("="*70);print(f"  ABLATION — config: {TAG}  |  active: {[k for k,v in CFG.items() if v]}");print("="*70,flush=True)
    ...
    print(f"  CalVal: {len(cv)}",flush=True)
    ...
    fold=os.path.join(EXPORT,TAG);os.makedirs(fold,exist_ok=True)
    tsc={}
    for c in TARGETS:
    ...
    for _,r in cal.iterrows():
        ...
        a=(chm_stats(chm) if CFG['chmstats'] else [])+[1. if r['Ftype']==k else 0. for k in FTYPE]+[1. if r['Altitude']==k else 0. for k in ALTI]
        arows.append(a)
    ...
    na=(6 if CFG['chmstats'] else 0)+len(FTYPE)+len(ALTI)
    model=Model(len(TARGETS),na,CFG).to(dev)
    ...
    plt.xlabel('Epoch');plt.ylabel('SmoothL1');plt.title(f"Ablation: {TAG}")
